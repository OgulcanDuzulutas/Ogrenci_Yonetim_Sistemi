...
class ogr_guncelle(Toplevel):
    def __init__(self):
      Toplevel.__init__(self)
      self.wm_geometry("600x400+400+100")
      self.wm_title("Öğrenci Güncelleme")
      self.wm_resizable(False, False)
        
      self.frame1 = Frame(self, height=100, bg="#f6eec9")
      self.frame1.pack(fill=X)
      self.frame2 = Frame(self, height=500, bg="#fed39f")
      self.frame2.pack(fill=X)

      Label(self.frame1, text="Öğrenci Numarasını Giriniz", bg="#f6eec9", 
            font=("Garamond", 15, "bold")).place(x=20, y=20)
      self.numarasi2 = Entry(self.frame1, font=("Garamond", 15, "bold"))
      self.numarasi2.place(x=300, y=20, width=250)

      Label(self.frame2, text="Öğrenci Adı", bg="#fed39f", font=("Garamond", 15, "bold")).place(x=60, y=10)
      Label(self.frame2, text="Öğrenci Soyadı", bg="#fed39f", font=("Garamond", 15, "bold")).place(x=60, y=40)
      
      Label(self.frame2, text="Öğrencilik Durumu", bg="#fed39f", font=("Garamond", 15, "bold")).place(x=60, y=70)
      Label(self.frame2, text="Bölümü", bg="#fed39f", font=("Garamond", 15, "bold")).place(x=60, y=100)
      Label(self.frame2, text="Doğum Tarihi", bg="#fed39f", font=("Garamond", 15, "bold")).place(x=60, y=130)
      Label(self.frame2, text="Adres", bg="#fed39f", font=("Garamond", 15, "bold")).place(x=60, y=160)
      
      self.ad = Entry(self.frame2, font=("Garamond", 15, "bold"))
      self.ad.place(x=300, y=10, width=250)
      self.soyadi = Entry(self.frame2, font=("Garamond", 15, "bold"))
      self.soyadi.place(x=300, y=40, width=250)
      
      self.sayi = IntVar(self.frame2)
      self.durum0 = Radiobutton(self.frame2, text="Aktif", variable=self.sayi, value=1)
      self.durum0.place(x=300, y=70)
      self.durum1 = Radiobutton(self.frame2, text="Dondurulmuş", variable=self.sayi, value=2)
      self.durum1.place(x=360, y=70)
      self.durum2 = Radiobutton(self.frame2, text="Mezun", variable=self.sayi, value=3)
      self.durum2.place(x=470, y=70)
      self.bolum = ttk.Combobox(self.frame2, font=("Garamond", 15, "bold"), state="readonly",
                                values=["BOTE", "BESYO", "TARİH", "MİMARLIK", "HUKUK", "TIP", "MATEMATİK", "KİMYA"])
      self.bolum.place(x=300, y=100, width=250)
      self.dogum_tarihi = DateEntry(self.frame2, font=("Garamond", 15, "bold"), locale="tr_TR")
      self.dogum_tarihi.place(x=300, y=130, width=250)
      self.adres = Text(self.frame2, font=("Garamond", 15, "bold"), height=3)
      self.adres.place(x=300, y=160, width=250)

      Button(self.frame1, text="ARAMA", command=self.arama,
             font=("Garamond", 15, "bold")).place(x=20, y=50, width=125)
      Button(self.frame2, text="GÜNCELLE", command=self.guncelle,
             font=("Garamond", 15, "bold")).place(x=20, y=250, width=125)
   
    def arama(self):
      sql = "SELECT * FROM ogrenci WHERE ogrenci_no=%s"
      val = (self.numarasi2.get(),)
      mycursor.execute(sql, val)
      ...
      for i in sonuc:
            ...
            self.soyadi.delete(0, END)
            self.adres.delete("1.0", END)
            self.dogum_tarihi.delete(0, END)
            self.sayi.set(0)

            self.ad.insert(0, i[1])
            self.soyadi.insert(0, i[2])
            self.sayi.set(i[4])
            self.bolum.set(i[5])
            self.dogum_tarihi.insert(0, i[6])
            self.adres.insert("end", i[7])
            
    def guncelle(self):
      sql = "UPDATE ogrenci SET bolum=%s,ogrenci_adi=%s,ogrenci_soyadi=%s,durum=%s, dogum_tarihi=%s, adres=%s " \
            "WHERE ogrenci_no=%s"
      val = (self.bolum.get(), self.ad.get(), self.soyadi.get(), self.sayi.get(),self.dogum_tarihi.get(),
             self.adres.get('1.0', END),self.numarasi2.get())
      mycursor.execute(sql, val)
      oysdb.commit()
      self.temizle()

    def temizle(self):
      messagebox.showinfo("Başarılı", "Güncelleme Başarılı")
      self.bolum.set("")
      self.ad.delete(0, END)
      self.soyadi.delete(0, END)
...
class ogr_sil(Toplevel):
    def __init__(self):
      ...
      Button(self.frame1, text="ARAMA", command=self.arama,
             font=("Garamond", 15, "bold")).place(x=20, y=50, width=125)
      Button(self.frame2, text="SİL", command=self.sil,
             font=("Garamond", 15, "bold")).place(x=20, y=250, width=125)
    ...
    def sil(self):
      ...
      ogrenci_no = self.numarasi2.get()

      sql = "DELETE FROM ogrenci WHERE ogrenci_no=%s"
      val = (ogrenci_no,)
      mycursor.execute(sql, val)
      oysdb.commit()
...
class ogr_arama(Toplevel):
    def __init__(self):
      ...
      Label(self.frame2, text="Öğrenci Adı", bg="#fed39f", font=("Garamond", 15, "bold")).place(x=60, y=10)
      Label(self.frame2, text="Öğrenci Soyadı", bg="#fed39f", font=("Garamond", 15, "bold")).place(x=60, y=40)
      Label(self.frame2, text="Öğrenci Numarası", bg="#fed39f", font=("Garamond", 15, "bold")).place(x=60, y=70)
      Label(self.frame2, text="Öğrencilik Durumu", bg="#fed39f", font=("Garamond", 15, "bold")).place(x=60, y=100)
      Label(self.frame2, text="Bölümü", bg="#fed39f", font=("Garamond", 15, "bold")).place(x=60, y=130)
      Label(self.frame2, text="Doğum Tarihi", bg="#fed39f", font=("Garamond", 15, "bold")).place(x=60, y=160)
      Label(self.frame2, text="Adres", bg="#fed39f", font=("Garamond", 15, "bold")).place(x=60, y=190)
      ...
      self.soyadi.place(x=300, y=40, width=250)
      self.numarasi = Entry(self.frame2, font=("Garamond", 15, "bold"))
      self.numarasi.place(x=300, y=70, width=250)
      self.sayi = IntVar(self.frame2)
      self.durum0 = Radiobutton(self.frame2, text="Aktif", variable=self.sayi, value=1)
      self.durum0.place(x=300, y=100)
      self.durum1 = Radiobutton(self.frame2, text="Dondurulmuş", variable=self.sayi, value=2)
      self.durum1.place(x=360, y=100)
      self.durum2 = Radiobutton(self.frame2, text="Mezun", variable=self.sayi, value=3)
      self.durum2.place(x=470, y=100)
      self.bolum = ttk.Combobox(self.frame2, font=("Garamond", 15, "bold"), state="readonly",
                                values=["BOTE", "BESYO", "TARİH", "MİMARLIK", "HUKUK", "TIP", "MATEMATİK", "KİMYA"])
      self.bolum.place(x=300, y=130, width=250)
      self.dogum_tarihi = DateEntry(self.frame2, font=("Garamond", 15, "bold"), locale="tr_TR")
      self.dogum_tarihi.place(x=300, y=160, width=250)
      self.adres = Text(self.frame2, font=("Garamond", 15, "bold"), height=3)
      self.adres.place(x=300, y=190, width=250)
